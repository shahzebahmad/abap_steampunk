...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<fs_entity>).
      legacy_entity_in = CORRESPONDING #( <fs_entity> MAPPING FROM ENTITY USING CONTROL ).
...
*         et_booking_supplement =
      ...
      ELSE.
        "fill filled return structure for the framework
  ...
  METHOD update.
    DATA: legacy_entity_in TYPE /dmo/travel,
          legacy_entity_x  TYPE /dmo/s_travel_inx, "Refers to X structure in BAPI
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).
      ...
      legacy_entity_x-_intx = CORRESPONDING zum_s_travel( <entity> MAPPING FROM ENTITY ).
...
*              _wait         =
        ...
        CATCH cx_abap_foreign_lock INTO DATA(ls_foreign_lock).
          "Filled failed return structure for the framework
          APPEND VALUE #( travelid = <key>-travelid ) TO failed-travel.
          ...
          APPEND VALUE #( travelid = <key>-travelid
                          %msg     = new_message( id   = '/DMO/CM_FLIGHT_LEGAC'
                                              number = '032'
                                              v1     = <key>-travelid
                                              v2     = ls_foreign_lock->user_name
                                              severity = CONV #( 'E' ) )
                        ) TO reported-travel.
*            C̣ATCH cx_abap_lock_failure.
      ENDTRY.
    ENDLOOP.
...
CLASS lsc_ZUM_I_TRAVEL DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZUM_I_TRAVEL IMPLEMENTATION.
